...
// Speak-request vars declared below near showSpeakRequestToast
...
function connect(){
  ...
  ws.onmessage=async e=>{
    ...
    switch(m.type){
      ...
      case 'hand_raised':
      case 'speak_request': {
        // Both the legacy raise_hand and new speak_request flow here
...
// ── Speak-request state vars ──────────────────────────────────────────────────
let speakRequestPending = false;   // participant: waiting for host approval
...
let activeSpeakerName   = null;    // host: name of currently approved speaker
let activeSpeakerCid    = null;    // host: cid of currently approved speaker

// Participant: send speak request to host
...
// Participant: host allowed → capture mic + stream to host only
...
// Participant: stop mic (called by host mute or by Done button)
...
// Participant: handle WebRTC answer from host for the speaker connection
...
// Host: receive participant mic offer + play their audio locally
...
// Host: clean up speaker connection
...
// Host: floating mute banner shown while participant is speaking
...
// Host: speak-request popup with Allow / Dismiss buttons
function showSpeakRequestToast(studentName, fromCid){
  ...
  inner.innerHTML=
    '<div style="display:flex;align-items:flex-start;gap:10px">'+
      ...
      '<div style="flex:1;min-width:0">'+
        ...
        '<div style="font-size:.73rem;color:#6366f1;margin-top:2px">Only you will hear them</div>'+
      '</div>'+
  ...
  card.addEventListener('pointerup',e=>{ dragging=false; if(e.clientX-startX>60) dismissCard(); else card.style.transform='translateX(0)'; });
  ...
  inner.querySelector('.hr-x').addEventListener('click',()=>{ clearTimeout(timer); dismissCard(); });
  ...
  inner.querySelector('.hr-dismiss').addEventListener('click',()=>{
    clearTimeout(timer);
    send({type:'speak_dismissed',toCid:fromCid}); dismissCard();
  });
